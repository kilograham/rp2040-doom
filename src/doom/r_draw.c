...
// todo graham revisit what combination of flags we should have these under
const byte *translationtables;
...
#if 0 // guess this was the old translation_tables
...
#if NO_USE_DC_COLORMAP
    should_be_const lighttable_t *dc_colormap = colormaps + 256 * dc_colormap_index;
#endif
...
void R_DrawPixel(int x, int y, int color) {
    ...
    if ((unsigned) x >= SCREENWIDTH)
        I_Error("R_DrawColumn: %i, %i", x, y);
#endif
    if (y < 0) return;
    if (y >= viewheight) return;
    pixel_t *dest = ylookup[y] + columnofs[x];
    *dest = color;
}
...
#if NO_USE_DS_COLORMAP
    should_be_const lighttable_t *ds_colormap = colormaps + ds_colormap_index * 256;
#endif
    do {
...
//        *dest++ = ((ytemp >> 6)&32) + ((xtemp >> 1)&16) + ((((uintptr_t)ds_source)>>2)&15);
...
void R_VideoClear() {
    memset(I_VideoBuffer, 255, SCREENWIDTH * viewheight);
}
...
void R_InitTranslationTables(void) {
    ...
    byte *tt = Z_Malloc(256 * 3, PU_STATIC, 0);
